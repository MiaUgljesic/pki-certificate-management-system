...
import { AdminCertificatesTable } from '../../../admin/components/admin-certificates-table/admin-certificates-table';
...
@Component({
  ...
  templateUrl: './organization-certificates.html',
  styleUrl: './organization-certificates.css',
})
export class OrganizationCertificates implements OnInit {
  ...
  private loadCertificates(page: number = 0, sortBy?: string, sortDir?: string, date?: number) {
    this.certificateService.getOrganizationCertificates(page, this.pageSize, sortBy, sortDir, date).subscribe({
      ...
      error: (error) => {
        console.error('Error loading organization certificates:', error);
      }
    });
  }
